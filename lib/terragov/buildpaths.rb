...
module Terragov
  # this class constructs a set of paths based upon
  # our agreed project structure
  class BuildPaths
    def base(options = {})
      ...
      secret_common_project_data = File.join(
        project_data_dir, 'common.secret.tfvars'
      )
      stack_project_data = File.join(project_data_dir, "#{stack}.tfvars")
      secret_project_data = File.join(
        project_data_dir, "#{stack}.secret.tfvars"
      )
    ...
    def check_var_files(paths = [])
      path_array = []
      paths.each { |p| path_array << data_validation(p) }

      return true if path_array.include?(true)
    ...
    def check_paths(paths)
      [paths[:terraform_dir], paths[:project_dir]].each do |path|
        abort("error: cannot find #{path}") unless data_validation(path)
      end

      # specific error for backend to remove ambiguity
      backend_err = [
        "error: cannot find backend file (#{paths[:backend_file]}).",
        'did you specify the right stack?'
      ].join("\n")

      abort(backend_err) unless data_validation(paths[:backend_file])
    end

    def build_command(options = {})
      check_paths(base(options))

      var_paths = data_paths(options)
      unless check_var_files(var_paths)
        abort('Error: cannot find any var files')
      end

      full_vars = []
      data_paths(options).each do |path|
        next unless data_validation(path)
        case path
        when paths[:secret_project_data], paths[:secret_common_project_data]
      ...
      # if defining additional terraform commands, they need to be passed in as
      # one string, with any hyphens escaped twice so it does not conflict
      # with commander CLI options
